...
from src.communication.opcua.opcua_client import PLCClient, Yaskawa_YRC1000
...
def main():
    ...
    print(f"Logging session folder created: {session_dir}")
    ...
    DEADBAND_M = 0.001  # 1 mm deadband
    LOOP_HZ = 30
    LOOP_DT = 1.0 / LOOP_HZ
    ...
    try:
        ...
        print(e)
    with PLCClient(plc_url) as plc, Yaskawa_YRC1000(robot_url) as robot:
        ...
        try:
            ...
            while True:
                ...
                if state_job in [11, 12, 13] and not logging_active:
                    trial_filename = session_dir / f"trial_{trial_id:03d}.csv"
                    csv_file = open(trial_filename, mode="w", newline="")
